...
export default async function handler(req, res) {
  ...
  try {
    ...
    const agentSearch = await airtableFetch(`${TABLE_AGENTS}?filterByFormula=${filterFormula}`, token);
    ...
    const dupSearch = await airtableFetch(`${TABLE_TRAINING_SESSIONS}?filterByFormula=${dupFormula}`, token);
    ...
    const sessionCreate = await airtableFetch(TABLE_TRAINING_SESSIONS, token, {
      method: "POST",
      body: JSON.stringify({
        records: [
          {
    ...
    await airtableFetch(TABLE_COACHING_REPORTS, token, {
      method: "POST",
      body: JSON.stringify({
        records: [
          {
